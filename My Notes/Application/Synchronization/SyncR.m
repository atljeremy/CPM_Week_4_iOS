...
+ (void)sendUnsyncedNotesToAPIWithCompletionBlock:(void (^)(NSError* error))completionBlock
{
    ...
    for (Note* note in notes) {
        ...
        AFHTTPRequestOperation* operation = [client HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            NSString* noteTitle = [responseObject objectForKey:kNoteTitleKey];
            NSString* noteDetails = [responseObject objectForKey:kNoteDetailsKey];
            Note* note = [Note noteWithCriteria:@{kNoteTitleKey: noteTitle, kNoteDetailsKey: noteDetails}];
            if (note) {
...
+ (void)deleteNotesFromAPI:(NSArray*)notes withCompletionBlock:(void (^)(NSError* error))completionBlock
{
    ...
    for (Note* note in notes) {
        ...
        AFHTTPRequestOperation* operation = [client HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            NSNumber* noteId = [responseObject objectForKey:kNoteIdKey];
            Note* note = [Note noteWithCriteria:@{kNoteAPIIdKey: noteId}];
            if (note) {
